...
import { retrieve, reset } from "../../actions/eventstructure/show";
import { del, loading, error } from "../../actions/eventstructure/delete";
import { list } from "../../actions/eventstructure/list";
    ...
    return (
      <View style={styles.actionStyle}>
        <SocialIcon
          ...
          type="plus-circle"
          iconColor="#3faab4"
          onPress={() => Actions.eventstructureCreate()}
        />
        <SocialIcon
          iconSize={34}
          type="edit"
          iconColor="#3faab4"
          onPress={() => Actions.eventstructureUpdate({ id })}
        />
        <SocialIcon
          iconSize={34}
          type="minus-circle"
  ...
  render() {
    ...
    return (
      <View style={{ flex: 1 }}>
        <ScrollView>
          {item && (
            <Card title="EventStructure">
              <List title="title">
                {Show.renderRow("id", item["@id"])}
                {Show.renderRow("name", item["name"])}
                {Show.renderRow("is_birth", item["is_birth"])}
                {Show.renderRow("isBirth", item["isBirth"])}
              </List>
            </Card>
          )}
...
const mapStateToProps = (state) => {
  return {
    error: state.eventstructure.show.error,
    loading: state.eventstructure.show.loading,
    retrieved: state.eventstructure.show.retrieved,
    deleteError: state.eventstructure.del.error,
    deleteLoading: state.eventstructure.del.loading,
