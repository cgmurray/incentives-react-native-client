...
import { retrieve, reset } from "../../actions/birth/show";
import { del, loading, error } from "../../actions/birth/delete";
import { list } from "../../actions/birth/list";
    ...
    return (
      <View style={styles.actionStyle}>
        <SocialIcon
          ...
          type="plus-circle"
          iconColor="#3faab4"
          onPress={() => Actions.birthCreate()}
        />
        <SocialIcon
          iconSize={34}
          type="edit"
          iconColor="#3faab4"
          onPress={() => Actions.birthUpdate({ id })}
        />
        <SocialIcon
          iconSize={34}
          type="minus-circle"
          ...
          {item && (
            <Card title="Birth">
              <List title="title">
                ...
                {Show.renderRow("stats", item["stats"])}
              </List>
            </Card>
          )}
...
const mapStateToProps = (state) => {
  return {
    error: state.birth.show.error,
    loading: state.birth.show.loading,
    retrieved: state.birth.show.retrieved,
    deleteError: state.birth.del.error,
    deleteLoading: state.birth.del.loading,
